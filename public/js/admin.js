...
async function loadAndShow() {
  ...
  DATA = pubData || {};
  // Use admin services endpoint (includes inactive) instead of public /api/data services
  DATA.services = servicesRes?.services || DATA.services || [];
...
function bindEvents() {
  ...
  document.getElementById('form-settings').addEventListener('submit', async e => {
    ...
    if (res.ok) {
      ...
      if (res.token) { TOKEN = res.token; localStorage.setItem('ae_admin_token', TOKEN); }
      toast('Настройки сохранены!');
      loadAndShow();
    } else toast('Ошибка сохранения', 'error');
  });

  // AUTH (Login form submit handler)
  document.getElementById('login-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch('/api/auth', {
        ...
        body: JSON.stringify({ username: user || undefined, password: pwd })
      });
      ...
      if (json.ok && json.token) { 
        ...
        if (err) {
          err.textContent = json.error || 'Неверный логин или пароль';
